...
namespace forest_escape {
	...
	void Renderer::update_projection_matrix(const float new_aspect_ratio)
	{
		...
		rengage::opengl_invoke(glUniformMatrix4fv, ARGS(m_proj_index, 1, GL_FALSE, glm::value_ptr(m_proj_matrix)));
	}
	...
	void Renderer::draw_model(const std::unique_ptr<rengage::model::Model>& model_ptr)
	{
		...
		rengage::opengl_invoke(glUniformMatrix4fv, ARGS(m_mv_index, 1, GL_FALSE, glm::value_ptr(model_view_matrix)));
		...
		{
			rengage::opengl_invoke(glBindVertexArray, ARGS(mesh.VAO().value()));
			...
			rengage::opengl_invoke(glDrawElements, ARGS(GL_TRIANGLES, mesh.indices().size(), GL_UNSIGNED_INT, nullptr));
