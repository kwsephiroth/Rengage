...
namespace rengage::shader
{
	std::unique_ptr<Shader> ShaderFactory::load_shader_from_file(const GLenum type, const std::string& filename)
	{
		...
		auto shader_id = ogl_invoker->get_invoke(glCreateShader, ARGS(type));
	...
	std::unique_ptr<Shader> ShaderFactory::load_shader_from_source(const GLenum type, const std::string& source)
	{
		std::unique_ptr<Shader> shader_ptr = nullptr;
		auto ogl_invoker = services::ServiceLocator::get_service<services::OGLInvoker>();
		GLenum shader_id = ogl_invoker->get_invoke(glCreateShader, ARGS(type));
	...
	GLint ShaderFactory::compile_shader_source(const GLuint shader_id, const std::string& source, const std::string& filename)
	{
		...
		auto ogl_invoker = services::ServiceLocator::get_service<services::OGLInvoker>();
		ogl_invoker->invoke(glShaderSource, ARGS(shader_id, 1, &source_cstr, nullptr));
		ogl_invoker->invoke(glCompileShader, ARGS(shader_id));
		GLint success;
		ogl_invoker->invoke(glGetShaderiv, ARGS(shader_id, GL_COMPILE_STATUS, &success));
		...
		if (success != GL_TRUE) {
			...
			ogl_invoker->invoke(glGetShaderInfoLog, ARGS(shader_id, MAX_LOG_LENGTH, &actual_log_length, info_log));
